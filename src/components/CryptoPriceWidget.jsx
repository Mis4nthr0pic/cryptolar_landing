...
const CryptoPriceWidget = ({ className = "" }) => {
  ...
  const createTextTexture = useCallback((text, color = '#ffffff', fontSize = 64) => {
  ...
  // Create price panel
  const createPricePanel = useCallback((coin, position) => {
    ...
    nameText.position.y = 0.5;
    nameText.position.z = 0.01;
    group.add(nameText);
    
    // Price text (will be updated)
    const priceTexture = createTextTexture('$0.00', '#ffffff', 40);
    ...
    const arrowTexture = createTextTexture('—', '#ffffff', 60);
    const arrowMaterial = new THREE.MeshBasicMaterial({
      map: arrowTexture,
      transparent: true
    });
    const arrowText = new THREE.Mesh(new THREE.PlaneGeometry(0.5, 0.4), arrowMaterial);
    arrowText.position.y = -0.5;
  ...
  const updatePanel = useCallback((panelGroup, priceData) => {
    ...
    // Update price text
    const formattedPrice = `$${current.toLocaleString('en-US', { 
      minimumFractionDigits: 2, 
      maximumFractionDigits: 2 
    })}`;
    
    const priceTexture = createTextTexture(formattedPrice, '#ffffff', 40);
    ...
    const arrowTexture = createTextTexture(arrowSymbol, arrowColor, 60);
    ...
    if (trend !== 'neutral') {
      ...
      const animate = () => {
        ...
        const opacity = glowIntensity * (1 - progress * 0.7);
        
        panel.material.opacity = Math.max(opacity, 0.05);
  ...
  useEffect(() => {
    ...
    const pointLight1 = new THREE.PointLight(0x627eea, 0.5, 10);
    pointLight1.position.set(-2, 2, 3);
    scene.add(pointLight1);
    
    const pointLight2 = new THREE.PointLight(0xf7931a, 0.5, 10);
    pointLight2.position.set(2, -2, 3);
    scene.add(pointLight2);

    // Create price panels
    const ethPanel = createPricePanel('ethereum', { x: -2, y: 0, z: 0 });
    const btcPanel = createPricePanel('bitcoin', { x: 2, y: 0, z: 0 });
    ...
    const animate = () => {
      ...
      if (ethPanelRef.current) {
        ethPanelRef.current.rotation.y += 0.005;
        ethPanelRef.current.rotation.x = Math.sin(Date.now() * 0.001) * 0.1;
      }
      
      if (btcPanelRef.current) {
        btcPanelRef.current.rotation.y -= 0.005;
        btcPanelRef.current.rotation.x = Math.cos(Date.now() * 0.001) * 0.1;
      }
  ...
  return (
    <div className={`crypto-price-widget relative ${className}`}>
      ...
      <div 
        ...
        style={{ background: 'linear-gradient(135deg, rgba(20,20,30,0.8) 0%, rgba(10,10,20,0.9) 100%)' }}
      />
      
      {/* Loading Overlay */}
      {isLoading && (
        <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 rounded-lg">
          <div className="text-white text-lg font-mono">Loading prices...</div>
        </div>
      )}
      
      {/* Status Bar */}
      <div className="mt-4 text-center">
        <div className="inline-flex items-center gap-4 bg-gray-900 bg-opacity-80 rounded-full px-6 py-2 text-sm font-mono">
